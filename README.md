...
For simpler tripod microwave outings with the IC-905 I can package a headless T4.0 in a snmall case and add a suitable interface transistors to drive antenna relays and switch PTT to the right amp.
 ...
 Apart from VFO, as a consequence of makiong the radio the master soruce, other parameters are read from the radio on band change ignoring my database copy so some code changes are needed to ignore the radio setting when the radio is an IF band/  When leaving an IF band the orignal settings for that directbad need to be restored.  The radio does not know how it is being used. Bandstack could be used but seems tricky to keep it from getting altered in normal use, plus it doens not have state for thigns like preamp and ATTN, AGC. Seems easier to use my own per band record.  The other option is to flip back to the controller is master source of settings always telling the radio what to set for the normal features.  Things like CW and memories are still radio side only. Some thought required.